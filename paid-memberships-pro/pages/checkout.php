...
<form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	...
	<thead>
		<tr>
			<th class="h1"><?php _e('Membership Level', 'pmpro');?></th>
		</tr>
		<?php if(count($pmpro_levels) > 1): ?>
			<tr>
				<th>
					 <a href="<?php echo pmpro_url("levels"); ?>"><?php _ex('Change level', 'change membership level link', 'pmpro');?></a>
				</th>						
			</tr>
		<?php endif; ?>
	</thead>
	<tbody>                
		<tr>
			<td>				
					<?php printf(__('<p>You have selected the <strong>%s</strong> membership level.</p><p>APS membership is for a one-year period and <strong>will renew automatically</strong>. A notice will be sent one month prior to renewal with the option to cancel membership or change your membership level.</p><p>Your membership is priced at:</p>', 'pmpro'), $pmpro_level->name);?> 
	...
	<thead>
		...
		<tr>
			<th>
				<?php _e('Already have an account?', 'pmpro');?> <a href="<?php echo wp_login_url(pmpro_url("checkout", "?level=" . $pmpro_level->id))?>"><?php _e('Log in here', 'pmpro');?></a>.
			</th>
		</tr>
	</thead>
	<tbody>                
	...
	<thead>
		...
		<tr>
			<th><p>This address will be used  solely for processing your membership payment. It will not appear online.  You will have the ability to add an optional contact mailing address to your public profile after you join.</p></th>
		</tr>
	</thead>
	<tbody>
		<tr>
	...
	<thead>
		<tr>
			<th class="h1"><?php _e('Payment Information', 'pmpro');?></th>
		</tr>
		<tr>
			<th><?php printf(__('We Accept %s', 'pmpro'), $pmpro_accepted_credit_cards_string);?></th>
		</tr>
	</thead>
	<tbody>                    
		<tr valign="top">		
